...
function AddBuyer() {
    ...
    const submitForm = (e) =>{
        e.preventDefault();
        setIsLoading(true);
        if (validateInputField({field : ownerName, fieldName : "owner name"}) && 
            ...
            axios[isUpdate ? "put" : "post"](url, body)
              .then(function (response) {
                ...
                if(status === 200){
                    resetForm();
                    displayToast({type : "success", msg : `${isUpdate ? ownerName + ' updated' : 'Buyer added'} successfully!`});
                    setIsLoading(false);
                    setTimeout(() => {
                         ...
                         <Card.Body>
                            <Form onSubmit={submitForm}>
                                <Form.Group className="mb-3" controlId="formBasicownerName">
                                    ...
                                    <Form.Control type="text" value={ownerName} onChange={handleOwnerNameChange} placeholder="Enter Buyer Name" />
                                </Form.Group>

                                <Form.Group className="mb-3" controlId="formBasicCompanyName">
                                    <Form.Label>Company Name</Form.Label>
                                    <Form.Control type="text" value={companyName} onChange={handleCompanyNameChange} placeholder="Enter Company Name" />
                                </Form.Group>

                                <Form.Group className="mb-3" controlId="formBasicZipcode">
                                    <Form.Label>Zipcode</Form.Label>
                                    <Form.Control type="text" value={zipcode} onChange={handleZipcodeChange} placeholder="Enter Zipcode" />
                                </Form.Group>
