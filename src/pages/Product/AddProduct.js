...
function AddProduct() {
    ...
    const submitForm = (e) =>{
        e.preventDefault();
        setIsLoading(true);
        if (validateInputField({field : productName, fieldName : "product name"}) && 
            validateInputField({field : quantity, fieldName : "quantity"}) &&
            validateInputField({field : price, fieldName : "price"})) {
            const body = {productName, quantity, price};
            ...
            axios[isUpdate ? "put" : "post"](url, body)
              .then(function (response) {
                ...
                if(status === 200){
                    resetForm();
                    displayToast({type : "success", msg : `${isUpdate ? productName + ' updated' : 'Product added'} successfully!`});
                    setIsLoading(false);
                    setTimeout(() => {
                            ...
                            <Form onSubmit={submitForm}>
                                <Form.Group className="mb-3" controlId="formBasicProductName">
                                    ...
                                    <Form.Control type="text" value={productName} 
                                        onChange={handleProductNameChange} placeholder="Enter Product Name" />
                                </Form.Group>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicPrice">
                                    ...
                                    <Form.Control type="number" value={price} 
                                        onChange={handlePriceChange} placeholder="Enter Price" />
                                </Form.Group>
